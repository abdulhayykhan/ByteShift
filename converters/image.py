...
async def image_to_docx(file: UploadFile, background_tasks: BackgroundTasks) -> FileResponse:
    """
    Convert an uploaded image to DOCX format.

    Args:
        file: Uploaded image file (PNG, JPG, WEBP)

    Returns:
        FileResponse with the image embedded in a DOCX document
    """
    ...
    # Create temporary input image file
    temp_image = tempfile.NamedTemporaryFile(delete=False)
    temp_image.write(contents)
    temp_image.close()
    image_path = temp_image.name

    # Create temporary output docx file
    ...
    try:
        ...
        doc = Document()
        doc.add_heading(os.path.splitext(file.filename)[0] if file.filename else "Image", level=1)
        doc.add_picture(image_path, width=5000000)  # 5 inches in EMU
        doc.save(output_path)
    except Exception as e:
        cleanup_file(output_path)
        raise ValueError(f"Failed to convert image to DOCX: {str(e)}") from e
    finally:
